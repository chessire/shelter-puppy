"""강아지 사진 앵커 — 고객 사진 몇 장으로 다견 영상의 강아지 트랙 자동 지정 (M2 재사용).
...
# *같은 영상 안* 트랙 간 격차. 단 격차는 **동시등장 트랙(=진짜 다른 강아지)**와만 비교 —
...
TAU_SIM = 0.40      # 절대 유사도 바닥 — 미만이면 "이 영상엔 그 강아지가 없을 수도"
TAU_MARGIN = 0.05   # 진짜 경쟁자와의 격차 하한 — 유사견종 오귀속(falseAttach) 방어
OVERLAP_EPS = 2     # 동시등장 ≤N프레임은 ID 스위치 순간 노이즈로 간주(같은 강아지 조각 허용)
FRAG_DELTA = 0.15   # top1 과 유사도 차 이내 + 비동시면 같은 강아지 조각으로 합침
...
def fragments_and_margin(sims: dict[int, float], frame_sets: dict[int, set],
    ...
    top1-delta 이내인 트랙을 같은 강아지 조각으로 합친다(시간상호배제의 역 — M2 신호 재사용).
    margin = top1 유사도 − 조각 밖(=동시등장한 진짜 다른 강아지) 최고 유사도.
    """
