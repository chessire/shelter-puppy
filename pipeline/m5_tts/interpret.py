...
def _match_scene(sentence: str, request: str) -> EditBlock:
    d = _chat_json(
        "강아지 영상 편집기다. 이 내레이션 문장이 나오는 동안 보여줄 장면 속성을 정해라.\n"
        ...
        "- subject: 강아지 당겨찍기·얼굴 중심=foster, 전체=full.\n"
        "- caption: 지시가 이 문장을 '텍스트로/자막으로 띄워라' 하면 그 문구"
        "(대개 내레이션 문장 그대로), 아니면 빈 문자열.\n"
        "- caption_pos: 지시가 자막 위치를 명시한 경우만(아래=bottom, 위=top, "
        "오른쪽=right, 왼쪽=left, 구석 top-left/top-right/bottom-left/bottom-right), "
        "아니면 bottom.\n"
...
def interpret_narration(request: str) -> EditPlan:
    ...
    """
    d = _chat_json(
        "강아지 영상 내레이션 요청이다. *목소리로 읽을 대본 문장*만 순서대로 뽑아라.\n"
