"""합침 B — M2(강아지 식별) ⊕ M3(모션곡선).

모션곡선을 전체화면이 아니라 *M2가 강아지으로 묶은 박스* 기준으로 계산한다.
다견 영상에서도 지정한 강아지 한 마리의 동/정만 뽑힌다.

검증: ROI 박스가 강아지(global_dog_id)을 프레임 내내 따라가는지 — 시각화로 확인.
(강아지 트랙 집합은 M2 결과에서; 여기선 M2 도출 GT로 '식별 성공'을 전제)
"""
...
def foster_boxes(pred_frames, foster_tracks: set[int]) -> dict[int, BBox]:
    """강아지 트랙들의 박스를 프레임별로 합쳐 ROI 궤적 생성(겹치면 최고 conf)."""
    out: dict[int, BBox] = {}
    best: dict[int, float] = {}
...
def main(argv=None) -> int:
    ...
    p.add_argument("--foster-dog", type=int, default=0, help="강아지 global id")
