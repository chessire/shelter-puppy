...
분석 산출물(관찰 프로필·동작 태그)은 입양 응대봇의 근거 DB(`rag/`)로도 활용된다.
...
영상 분석 산출물을 입양 응대봇의 근거 DB로 변환한다 (`rag/`). pgvector(Postgres) 단일
스택, bge-m3(1024차원) 임베딩, 모든 쿼리에 `dog_id` 필터. (여기의 L1/L2는 RAG 계층명 —
화면 텍스트 3레벨과 무관.)
...
핵심 규칙 — **저장 위치별로 텍스트 출처를 고정한다**. 환각 위험은 conf(검출 확신도)가
아니라 그 문장을 누가 만들었느냐에 따라 달라지므로, 청크마다 출처 컬럼을 두는 대신
테이블·layer 단위로 출처를 통일한다: 정형 카드=사람이 입력한 문장 / L2=관찰 문장
원문(LLM 수정 금지) / L1=LLM이 문장을 생성하는 유일한 곳(사실 검수 필수). 영상에 들어간
저작 자막·내레이션은 DB에 넣지 않는다 — 같은 내용의 관찰 기록이 이미 있고, 자막 검수는
창작 문장을 사실로 보증하는 게 아니기 때문.
